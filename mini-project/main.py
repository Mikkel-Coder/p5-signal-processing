...
from numpy import abs, float64, complex128, convolve
...
# Our input signal
input_signal = lambda n: cos((pi/8)*n)+cos((3*pi/4)*n)
...
def _plot_freq(w: list[float], h: list[complex128], window_name: str, path: str) -> None:
...
for window, window_name in windows:
    ...
    #/windows_name/freq/input/L.png
    #/windows_name/freq/output/
    ...
    for L in range(MIN_L, MAX_L+1, 2):
        ...
        high = _find_magnitude_at_angular_frequency(passband_angular_frequency, w, h)
        low = _find_magnitude_at_angular_frequency(stopband_angular_frequency, w, h)
        ...
        fixed_w = [-q for q in w[::-1] ]+w
        fixed_h = [q for q in h[::-1] ]+h
        ...
        if high > passband_magnitude and low < stopband_magnitude:
            ...
            _plot_freq(fixed_w, freq_input_signal, window_name, freq_input_path)
            _plot_freq(w, h, window_name, freq_transferfunction_path)
            _plot_freq(fixed_w, freq_output_signal, window_name, freq_output_path)
